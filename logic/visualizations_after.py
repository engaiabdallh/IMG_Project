...
def visualize_dataset_af(df, numeric_cols=None, categorical_cols=None):
    if numeric_cols:
...
def plot_boxplots_af(df, numeric_cols):
    ...
    for i, col in enumerate(numeric_cols):
        with cols[i % 4]:
            fig, ax = plt.subplots()
            if col == "fare":
                sns.boxplot(y=df[col], ax=ax, color='lightgreen', showfliers=False)  # Hide outliers for 'fare'
            else:
...
def plot_correlation_heatmap_af(df_numeric):
...
def visualize_categorical_af(df, categorical_cols):
